<?php
    ...
    function hang_hoa_select_all_adidas(){
        $sql = "SELECT * FROM hang_hoa JOIN loai_hang ON hang_hoa.ma_loai = loai_hang.ma_loai WHERE ten_loai='Adidas'";// kết hợp giữ hàng hoá và loại hàng dựa trên cột mã loại
        return pdo_query($sql);
    }
    ...
    function hang_hoa_select_all_price(){
        $sql = "SELECT * FROM hang_hoa ORDER BY don_gia DESC LIMIT 0,9"; // limit giới hạn kết quả trả về
        return pdo_query($sql);
    }
    ...
    function hang_hoa_select_keyword($keyword){
        $sql = "SELECT * FROM hang_hoa hh "
                ...
                . " WHERE ten_hh LIKE ? OR ten_loai LIKE ?"; //like là mệnh đề dùng để tìm kiếm một mẫu cụ thể trong một cột
        return pdo_query($sql, '%'.$keyword.'%', '%'.$keyword.'%'); // % đại diện cho không, một hoặc nhiều ký tự
    }
